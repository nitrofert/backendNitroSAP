...
const opcionesSubMenu = await db.query(`SELECT DISTINCT t0.* 
    ...
    async getNextLineAprovedEntrada(idEntrada: number, 
        bdmysql: string, 
        companysap: string, 
        logo: string,
        origin:string='http://localhost:4200',
        ...
        const nextLineAprovedEntrada: any[] = await db.query(queryNextApprovedLine);
    ...
    async DetalleAprobacionEntrada(idEntrada: number, bdmysql: string): Promise<any> {
        ...
        //console.log(query);
        const detalleAprobacionEntrada: any[] = await db.query(query);
    ...
    async loadBodyMailEntradaAp(infoUsuario:any, LineAprovedEntrada: any, logo: string, entrada: any, key: string, urlbk:string,accionAprobacion?:boolean,verBotones?:boolean): Promise<string> {

        const entradaDet: any[] = entrada.DocumentLines;
        console.log('LineAprovedEntrada',LineAprovedEntrada);
        
        let subtotal = 0;
        let totalimpuesto = 0;
        let total = 0;
        const detalleAprobacionEntrada = await helper.DetalleAprobacionEntrada(entrada.id, infoUsuario.bdmysql);
        
        let htmlDetalleAprobacion = '';
        let lineaDetalleAprobacion = '';
        let serieNombre = "";
        let seriesDoc = await helper.getSeriesXE(infoUsuario.dbcompanysap,'20');
        for(let item in seriesDoc) {
                                                    ...
                                                    <table role="presentation" style="width:100%;border-collapse:collapse;border:0;border-spacing:0;">
                                                        <tr>
                                                            ...
                                                            <td style="width:50%;">
                                                                ...
                                                                <span style="font-size:smaller;padding-left: 3px;">${serieNombre}</span><br />
                                                                <span style="font-weight: bold; font-size: smaller; padding-left: 2px;">Fecha contabilización / Fecha expira </span><br />
                                                                <span style="font-size:smaller;padding-left: 3px;">${await helper.format(entrada.DocDate)} - ${ await helper.format(entrada.DocDueDate)}</span><br />
                                                                <span style="font-weight: bold; font-size: smaller; padding-left: 2px;">Fecha ducumento / Fecha necesaria </span><br />
                                                                <span style="font-size:smaller;padding-left: 3px;">${await helper.format(entrada.TaxDate)} - ${await helper.format(entrada.reqdate)}</span><br />
                                                            </td>
                                                        </tr>
    ...
    async loadBodyMailApprovedEntrada(infoUsuario:any,
        LineAprovedEntrada: any, 
        logo: string, 
        entrada: any, 
        key: string, 
        urlbk:string,
...
const entradaDet: any[] = entrada.EntradaDet;

let subtotal = 0;
let totalimpuesto = 0;
let total = 0;
const detalleAprobacionEntrada = await helper.DetalleAprobacionEntrada(entrada.entrada.id, LineAprovedEntrada.infoEntrada.bdmysql);
let htmlDetalleAprobacion = '';
let lineaDetalleAprobacion = '';
let serieNombre =  "";
let seriesDoc = await helper.getSeriesXE(infoUsuario.dbcompanysap,'20');
...
return html;
    ...
    async modeloAprobacionesMysql(infoUsuario: InfoUsuario, area:string, tipo:string ='SOL'):Promise<any> {
        ...
        const modelos = await db.query(query_modelos);
        const data2 = modelos;
    ...
    async getEntradaById(idEntrada: string, bdmysql: string): Promise<any> {
        ...
        const entradaResult: any[] = await db.query(query);
    ...
    async fechaInicioSemana(fecha:Date):Promise<Date>{
      ...
      async udoUsuarioSL(infoUsuario: any, codusersap: string): Promise<any> {
    ...
    async udoUsuarioByIDSL(infoUsuario: any, idUsuarioSAP: string): Promise<any> {
    ...
    async getUsuarioSAPSL(infoUsuario: any, codusersap: string): Promise<any> {
    ...
    async registerUdoSAPSL(infoUsuario: any, data: any): Promise<any> {
    ...
    async updateUdoSAPSL(infoUsuario: any, data: any, docEntry:any, tipoUpdate:string): Promise<any> {
    ...
    async RegisterAreasUserMSQL(areasUser:any):Promise<any>{

        try{

            let result = await db.query(`insert into areas_user (codusersap,area,companyid,userid) values  ?`, [areasUser]);
    ...
    async RegisterDependenciasUserMSQL(dependenciesUser:any):Promise<any>{

        try{

            let result = await db.query(`insert into dependencies_user (codusersap,dependence,location,vicepresidency,companyid,userid) values  ?`, [dependenciesUser]);
    ...
    async RegisterAlmacenesUserMSQL(almacenesUser:any):Promise<any>{

        try{

            let result = await db.query(`insert into stores_users (codusersap,store,companyid,userid) values  ?`, [almacenesUser]);
    ...
    async deleteAreasUserMSQL(areasUser:any):Promise<any>{

        let idareas = areasUser.map((area: { id: any; })=>{
            return area.id;
        })

        try{

            let result = await db.query(`delete  from areas_user where id in(?)`, [idareas]);
    ...
    async deleteAlmacenesUserMSQL(almacenesUser:any):Promise<any>{

        let idalmacenes = almacenesUser.map((almacen: { id: any; })=>{
            return almacen.id;
        })

        try{

            let result = await db.query(`delete  from stores_users where id in(?)`, [idalmacenes]);
    ...
    async deleteDependenciasUserMSQL(dependenciasUser:any):Promise<any>{

        let iddependencias = dependenciasUser.map((dependencia: { id: any; })=>{
            return dependencia.id;
        })

        try{

            let result = await db.query(`delete  from dependencies_user where id in(?)`, [iddependencias]);
    ...
    async getAreasUserSAP(userSAP:string, bdmysql:string):Promise<any>{
        ...
        try{
            ...
            WHERE t0.codusersap = '${userSAP}' AND t1.urlwsmysql = '${bdmysql}'`);
